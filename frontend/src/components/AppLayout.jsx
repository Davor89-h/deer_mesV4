...
import {
  ...
  Code2, Calendar, Monitor, Layers, Hammer,
...
const NAV_ITEMS = [
  ...
  { to:'/kpi', key:'kpi', Icon:BarChart2, group:'v6', label:'KPI Dashboard' },
  { to:'/sales', key:'sales', Icon:ShoppingCart, group:'v6', label:'Sales' },
  { to:'/quality', key:'quality', Icon:CheckSquare, group:'v6', label:'Kvaliteta (QS)' },
  { to:'/warehouse', key:'warehouse', Icon:Warehouse, group:'v6', label:'Skladište' },
  { to:'/hr', key:'hr', Icon:UserCircle, group:'v6', label:'HR' },
  { to:'/dms', key:'dms', Icon:FolderOpen, group:'v6', label:'DMS' },
...
const GROUP_LABELS = { v6:'▶ MES v6', fixture:'STROJEVI', mes:'ALATI & NAPRAVE', ai:'ML / AI', workflow:'WORKFLOW' }
  ...
  return (
    <div style={{ display:'flex',minHeight:'100vh',background:C.bg,fontFamily:"'Chakra Petch',sans-serif" }}>
      <aside style={{ width:sideW,background:`linear-gradient(180deg,${C.surface} 0%,${C.surface2} 100%)`,borderRight:`1px solid ${C.border}`,display:'flex',flexDirection:'column',position:'fixed',top:0,bottom:0,left:0,zIndex:100,transition:'width 0.25s ease',overflow:'hidden',boxShadow:`4px 0 24px rgba(0,0,0,.25)` }}>
        ...
        <nav style={{ flex:1,overflowY:'auto',overflowX:'hidden',padding:'8px 0',display:'flex',flexDirection:'column',gap:1 }}>
          {NAV_ITEMS.map((item) => {
            ...
            return (
              <div key={item.to}>
                {showSep && expanded && <div style={{ padding:'10px 16px 4px',fontSize:9,color:C.muted2,letterSpacing:2,marginTop:4 }}>{GROUP_LABELS[item.group]}</div>}
                {showSep && !expanded && <div style={{ margin:'6px 10px',height:1,background:`${C.border}66` }}/>}
                <NavLink to={item.to} end={item.exact} title={label}
                  style={({isActive})=>({
                    display:'flex',alignItems:'center',gap:10,
                    margin:'1px 8px',padding:expanded?'9px 12px':'9px',
                ...
                >
                  ...
                  {expanded && <span style={{ fontSize:12,fontWeight:500,whiteSpace:'nowrap',letterSpacing:0.3 }}>{label}</span>}
                </NavLink>
              </div>
            )
          })}
      ...
      <div style={{ marginLeft:sideW,flex:1,display:'flex',flexDirection:'column',minWidth:0,transition:'margin-left 0.25s ease' }}>
        <header style={{ height:58,background:`linear-gradient(90deg,${C.surface},${C.surface2})`,borderBottom:`1px solid ${C.border}`,display:'flex',alignItems:'center',padding:'0 24px',gap:12,position:'sticky',top:0,zIndex:50,boxShadow:'0 2px 16px rgba(0,0,0,.2)' }}>
          <div>
            <div style={{ fontSize:9,color:C.muted,letterSpacing:2 }}>DEER MES v6 · INDUSTRIAL AI</div>
            <div style={{ fontSize:15,fontWeight:700,color:'#E8F2F0',letterSpacing:2 }}>{pageTitle}</div>
          </div>
          <div style={{ flex:1 }}/>
          <LanguageSwitcher compact/>
          <button onClick={()=>setNotifOpen(o=>!o)}
            style={{ width:36,height:36,borderRadius:10,border:`1px solid ${unread>0?C.accent:C.border}`,background:C.surface3,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',position:'relative',transition:'all .2s' }}>
        ...
        <main style={{ padding:24,flex:1 }}><Outlet/></main>
      </div>

      <div style={{ position:'fixed',right:notifOpen?0:-360,top:58,bottom:0,width:340,background:`linear-gradient(180deg,${C.surface},${C.surface2})`,borderLeft:`1px solid ${C.border}`,zIndex:80,transition:'right 0.3s',overflowY:'auto',boxShadow:notifOpen?'-8px 0 32px rgba(0,0,0,.4)':'none' }}>
        <div style={{ padding:'16px 18px',borderBottom:`1px solid ${C.border}44`,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,background:C.surface }}>
          <span style={{ fontWeight:700,fontSize:13,letterSpacing:1.5,color:'#E8F2F0' }}>OBAVIJESTI {unread>0&&<span style={{color:C.orange}}>({unread})</span>}</span>
        ...
        {alerts.map((a,i)=>(
          <div key={a.id||i} onClick={()=>markRead(a.id)}
            style={{ padding:'14px 18px',borderBottom:`1px solid ${C.border}33`,display:'flex',gap:11,cursor:'pointer',opacity:a.is_read?.55:1,transition:'background .15s' }}
